...
export default async function LockinArticlePage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  ...
  const storeUrl = `https://apps.apple.com/${cc}/app/lockin/id6802325423`;
  const isRtl = locale === "ar" || locale === "he";

  const content = article.content.replace(
    /https:\/\/apps\.apple\.com\/(?:[a-z]{2}(?:-[A-Za-z]+)?\/)?app\/(?:lockin\/)?id6802325423/g,
    storeUrl,
  );
  ...
  return (
    <main className="min-h-screen bg-[#050506] text-white" dir={isRtl ? "rtl" : undefined}>
      ...
      <header className="px-6 pt-12 pb-10 sm:pt-16 sm:pb-14 max-w-4xl mx-auto">
        <div className="flex items-center gap-3 text-sm font-medium uppercase tracking-wider text-[#FF3B30]">
      ...
      <div className="px-6 max-w-4xl mx-auto mb-6">
        ...
        >
          ...
          <div className="relative flex items-center justify-between gap-4 px-6 py-5 sm:px-8">
            <div>
              <span className="text-[15px] font-semibold text-white">Lock In!</span>
              <span className="block mt-1 text-[13px] text-neutral-400">Block distracting websites and apps during focus sessions — system-wide.</span>
            </div>
            <span
      ...
      <article className="px-6 py-12 sm:py-16 max-w-4xl mx-auto">
        <div
          className={[
            ...
            "[&_a]:text-[#FF3B30] [&_a]:hover:text-[#ff6259] [&_a]:underline [&_a]:underline-offset-2",
            ...
            "[&_blockquote]:border-l-4 [&_blockquote]:border-[#FF3B30] [&_blockquote]:pl-5 [&_blockquote]:italic [&_blockquote]:text-neutral-300 [&_blockquote]:my-6",
          ].join(" ")}
        ...
        <section className="px-6 pb-8 max-w-4xl mx-auto">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            {related.map((a) => (
              <Link
                key={a.slug}
                href={`/lockin/blog/${locale}/${a.slug}`}
                className="group block rounded-2xl border border-neutral-800 bg-neutral-900/50 p-6 transition-all hover:border-[#FF3B30]/60 hover:bg-neutral-900 hover:shadow-xl hover:shadow-[#FF3B30]/5"
              >
                <h3 className="font-serif text-lg font-bold text-white leading-snug group-hover:text-[#FF3B30] transition-colors">
                  {a.title}
                </h3>
      ...
      <section className="px-6 py-20 max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="relative">
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-white tracking-tight">Lock In!</h2>
            <p className="mt-4 text-lg text-neutral-400 max-w-md mx-auto leading-relaxed">
              Block distracting websites and apps during focus sessions — system-wide.
            </p>
            <a
              href={storeUrl}
              target="_blank"
            ...
            >
              Get Lock In! on the Mac App Store
      ...
      <footer className="px-6 py-10 max-w-4xl mx-auto border-t border-neutral-800">
        <nav className="flex items-center justify-center gap-6 text-sm text-neutral-500" dir="ltr">
          <Link href={`/lockin/blog/${locale}`} className="hover:text-white transition">Blog</Link>
          <Link href="/lockin/privacy" className="hover:text-white transition">Privacy</Link>
          <Link href="/lockin/terms" className="hover:text-white transition">Terms</Link>
        </nav>
      </footer>
    </main>
  );
