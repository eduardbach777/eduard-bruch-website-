...
export default async function FisheyeArticlePage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  ...
  const storeUrl = buildStoreUrl({
    appSlug: "fisheye-camera-wideeye",
    appId: "6760895260",
    cc,
    campaign: campaignFor.article("fisheye"),
  });
  const isRtl = locale === "ar" || locale === "he";

  const content = article.content.replace(
    /https:\/\/apps\.apple\.com\/(?:[a-z]{2}(?:-[A-Za-z]+)?\/)?app\/(?:fisheye-camera-wideeye\/)?id6760895260/g,
    storeUrl,
  );
    ...
    <main className="min-h-screen bg-[#050506] text-white" dir={isRtl ? "rtl" : undefined}>
      ...
      <div className="px-6 max-w-4xl mx-auto mb-6">
        ...
        >
          ...
          <div className="relative flex items-center justify-between gap-4 px-6 py-5 sm:px-8">
            <div>
              <span className="text-[15px] font-semibold text-white">Fisheye Camera</span>
              <span className="block mt-1 text-[13px] text-neutral-400">Live fisheye photo and video camera for iPhone — no clip-on lens.</span>
            </div>
            <span
        ...
        <section className="px-6 pb-8 max-w-4xl mx-auto">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            {related.map((a) => (
              <Link
                key={a.slug}
                href={`/fisheye/blog/${locale}/${a.slug}`}
      ...
      <section className="px-6 py-20 max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="relative">
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-white tracking-tight">Fisheye Camera</h2>
            <p className="mt-4 text-lg text-neutral-400 max-w-md mx-auto leading-relaxed">
              Live fisheye photo and video camera for iPhone — no clip-on lens.
            </p>
            <a
              href={storeUrl}
              target="_blank"
            ...
            >
              Get Fisheye Camera on the App Store
      ...
      <footer className="px-6 py-10 max-w-4xl mx-auto border-t border-neutral-800">
        <nav className="flex items-center justify-center gap-6 text-sm text-neutral-500" dir="ltr">
          <Link href={`/fisheye/blog/${locale}`} className="hover:text-white transition">Blog</Link>
          <Link href="/fisheye/privacy" className="hover:text-white transition">Privacy</Link>
          <Link href="/fisheye/terms" className="hover:text-white transition">Terms</Link>
        </nav>
      </footer>
    </main>
  );
