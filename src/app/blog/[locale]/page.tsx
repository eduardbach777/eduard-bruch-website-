...
export function generateStaticParams() {
  return BLOG_LOCALES.map((l) => ({ locale: l.code }));
}
...
export async function generateMetadata({
  ...
  return {
    title: `${l.heading} — Eduard Bruch`,
    description: l.subtitle,
    alternates: {
      languages: Object.fromEntries(
        BLOG_LOCALES.map((loc) => [loc.code, `/blog/${loc.code}`]),
      ),
    },
  };
}
              ...
              {app.articles.map((article: ArticleCard) => (
                ...
                >
                  ...
                  <span className="mt-4 inline-flex items-center gap-1 text-[13px] font-semibold text-[#d4ad5e] transition-all group-hover:gap-2">
                    Read more
                    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d={isRtl ? "M15 19l-7-7 7-7" : "M9 5l7 7-7 7"} /></svg>
                  </span>
                </Link>
              ))}
            </div>
