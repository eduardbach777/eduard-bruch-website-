...
export default async function BellowsArticlePage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  ...
  const content = article.content.replace(
    /https:\/\/apps\.apple\.com\/(?:[a-z]{2}(?:-[A-Za-z]+)?\/)?app\/(?:bellows\/)?id6805032839/g,
    storeUrl,
  );
  ...
  return (
    <main className="min-h-screen bg-[#050506] text-white" dir={isRtl ? "rtl" : undefined}>
      ...
      <header className="px-6 pt-12 pb-10 sm:pt-16 sm:pb-14 max-w-4xl mx-auto">
        <div className="flex items-center gap-3 text-sm font-medium uppercase tracking-wider text-[#FF9F0A]">
      ...
      <div className="px-6 max-w-4xl mx-auto mb-6">
        ...
        >
          ...
          <div className="relative flex items-center justify-between gap-4 px-6 py-5 sm:px-8">
            <div>
              <span className="text-[15px] font-semibold text-white">Bellows</span>
              <span className="block mt-1 text-[13px] text-neutral-400">41 developer tools in one native Mac app — JSON, JWT, Base64, hashes, and more.</span>
            </div>
            <span
      ...
      <article className="px-6 py-12 sm:py-16 max-w-4xl mx-auto">
        <div
          className={[
            ...
            "[&_a]:text-[#FF9F0A] [&_a]:hover:text-[#ffb23d] [&_a]:underline [&_a]:underline-offset-2",
            ...
            "[&_blockquote]:border-l-4 [&_blockquote]:border-[#FF9F0A] [&_blockquote]:pl-5 [&_blockquote]:italic [&_blockquote]:text-neutral-300 [&_blockquote]:my-6",
          ].join(" ")}
        ...
        <section className="px-6 pb-8 max-w-4xl mx-auto">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            {related.map((a) => (
              <Link
                key={a.slug}
                href={`/bellows/blog/${locale}/${a.slug}`}
                className="group block rounded-2xl border border-neutral-800 bg-neutral-900/50 p-6 transition-all hover:border-[#FF9F0A]/60 hover:bg-neutral-900 hover:shadow-xl hover:shadow-[#FF9F0A]/5"
              >
                <h3 className="font-serif text-lg font-bold text-white leading-snug group-hover:text-[#FF9F0A] transition-colors">
                  {a.title}
                </h3>
      ...
      <section className="px-6 py-20 max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="relative">
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-white tracking-tight">Bellows</h2>
      ...
      <footer className="px-6 py-10 max-w-4xl mx-auto border-t border-neutral-800">
        <nav className="flex items-center justify-center gap-6 text-sm text-neutral-500" dir="ltr">
          <Link href={`/bellows/blog/${locale}`} className="hover:text-white transition">Blog</Link>
          <Link href="/bellows/privacy" className="hover:text-white transition">Privacy</Link>
          <Link href="/bellows/terms" className="hover:text-white transition">Terms</Link>
        </nav>
      </footer>
    </main>
  );
