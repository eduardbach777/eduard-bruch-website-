...
export default async function JettyArticlePage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  ...
  const content = article.content.replace(
    /https:\/\/apps\.apple\.com\/(?:[a-z]{2}(?:-[A-Za-z]+)?\/)?app\/(?:jetty\/)?id6806639647/g,
    storeUrl,
  );
    ...
    <main className="min-h-screen bg-[#050506] text-white" dir={isRtl ? "rtl" : undefined}>
      ...
      <div className="px-6 max-w-4xl mx-auto mb-6">
        ...
        >
          ...
          <div className="relative flex items-center justify-between gap-4 px-6 py-5 sm:px-8">
            <div>
              <span className="text-[15px] font-semibold text-white">Jetty</span>
              <span className="block mt-1 text-[13px] text-neutral-400">Launch any app from the menu bar — grouped panels, one click.</span>
            </div>
            <span
      ...
      <section className="px-6 py-20 max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="relative">
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-white tracking-tight">Jetty</h2>
            ...
            >
              Get Jetty on the Mac App Store
      ...
      <footer className="px-6 py-10 max-w-4xl mx-auto border-t border-neutral-800">
        <nav className="flex items-center justify-center gap-6 text-sm text-neutral-500" dir="ltr">
          <Link href={`/jetty/blog/${locale}`} className="hover:text-white transition">Blog</Link>
          <Link href="/jetty/privacy" className="hover:text-white transition">Privacy</Link>
          <Link href="/jetty/terms" className="hover:text-white transition">Terms</Link>
        </nav>
      </footer>
    </main>
  );
